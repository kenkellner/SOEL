...
#font_install('fontcm')
#loadfonts()
pdf(file="../dissertation/figures/fig1-1.pdf",width=3.9,height=5,family="CM Roman",pointsize=9)
par(mfrow=c(2,1),mar=c(4,4,3,2))
hist(data.bo, breaks=15,xlim=c(0,100),main="Black Oak", col="gray38", xlab="dbh", freq=FALSE)
abline(v=median(data.bo,na.rm=TRUE),lwd=3)
hist(data.sm, breaks=15,xlim=c(0,100), main="Sugar Maple", col="gray75", xlab="dbh", freq=FALSE)
abline(v=median(data.sm),lwd=3)
dev.off()
